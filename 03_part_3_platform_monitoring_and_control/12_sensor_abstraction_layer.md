...
```mermaid
flowchart TB
    A["硬體待測電壓 Rail"] --> B["分壓電阻或濾波電路"]
    B --> C["BMC SoC ADC pin / ADC channel"]
    C --> D["Linux IIO ADC driver"]
    D --> E["iio-hwmon bridge"]
    E --> F["/sys/class/hwmon/hwmonX/inY_input"]
    F --> G["OpenBMC adcsensor daemon"]
    G --> H["Entity Manager configuration"]
    H --> I["D-Bus voltage sensor object"]
    I --> J["Redfish / WebUI / IPMI SDR / logging / threshold event"]
```
