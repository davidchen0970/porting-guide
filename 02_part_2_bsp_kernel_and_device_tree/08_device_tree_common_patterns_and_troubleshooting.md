...
- Linux hwmon subsystem 將溫度資料提供給 user space。
...
DTS / DTSI
   ...
   DTB
    │
    │ Bootloader 載入並傳給 kernel
    ▼
Linux Kernel
    │
    ├─ 解析 Device Tree
    ├─ 建立 platform / bus devices
    ├─ 依 compatible 尋找 driver
    └─ 執行 driver probe
```
...
SoC DTSI ─────────────┐
                      │
Common board DTSI ────┼──> Board DTS ──dtc──> DTB
                      │                    │
Board-specific DTS ───┘                    │
                                           ▼
                                      Bootloader
...
- Line name：提供人類可讀名稱，方便排查與 user space 對照。
...
- 若資訊會依產品政策、SKU 或現場設定改變，通常應留在 user space。
...
.bbappend 已套用
    ↓
SRC_URI 已包含 DTS / patch
    ↓
Kernel workdir 具有正確 DTS
    ↓
DTB 已重新編譯與 deploy
    ↓
DTB 已進入 FIT / flash image
    ↓
Bootloader 載入新 DTB
    ↓
Kernel running DTB 符合預期
```
...
- [ ] GPIO line、offset、active level、physical level、logical value已實測。
...
6. Device Tree 放固定硬體描述；threshold、fan policy、service policy 與產品功能選擇通常留在 OpenBMC user space 或 product layer。
